...
export default function FinanceTracker() {
  ...
  useEffect(() => {
    ...
    const checkAndCreateMonthlyReminders = async () => {
      ...
      for (const reminder of reminders) {
        ...
        // Verificar si el día de vencimiento coincide con hoy
        const dueDateDay = dueDate.getDate();
        
        // Si hoy es día 1 y el recordatorio es para día 1, O si hoy coincide con el día del recordatorio
        if ((dayOfMonth === 1 && dueDateDay === 1) || (dayOfMonth === dueDateDay)) {
          try {
            ...
            // Verificar si alguna transacción es del mes actual
            const hasCurrentMonthTransaction = existingTransactions.docs.some(doc => {
              const txDate = new Date(doc.data().date);
            ...
            if (!hasCurrentMonthTransaction) {
              ...
              console.log(`✅ Transacción mensual creada automáticamente para: ${reminder.name}`);
    ...
    // Ejecutar cada hora para verificar
  ...
  const filterRemindersByDate = () => {
    const now = new Date(selectedReminderDate);
  ...
  const filterReminders = () => {
    const dateFiltered = filterRemindersByDate();
  ...
  // Calcular estadísticas de recordatorios filtrados por fecha
  const filteredRemindersForStats = filterRemindersByDate();
  const totalRemindersFiltered = filteredRemindersForStats.length;
  const pendingRemindersFiltered = filteredRemindersForStats.filter(r => r.status === 'pendiente');
  const totalPendingFiltered = pendingRemindersFiltered.reduce((sum, r) => sum + r.amount, 0);
  const overdueRemindersFiltered = pendingRemindersFiltered.filter(r => getDaysUntilDue(r.dueDate) < 0);
  const dueSoonRemindersFiltered = pendingRemindersFiltered.filter(r => {
          ...
          <>
            ...
            <div className="bg-white rounded-lg shadow-md p-4 sm:p-6 mb-4 sm:mb-6">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <h3 className="text-base sm:text-lg font-bold text-gray-800">Filtrar por Fecha</h3>
                <div className="flex flex-wrap gap-2 w-full sm:w-auto">
                  <button
                  ...
                  <input
                    type="date"
                    value={selectedReminderDate.toISOString().split('T')[0]}
                    onChange={(e) => setSelectedReminderDate(new Date(e.target.value))}
                    className="px-3 sm:px-4 py-2 border border-gray-300 rounded-lg text-xs sm:text-sm w-full sm:w-auto"
                  />
                </div>
              </div>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-4 mb-4 sm:mb-6">
              <div className="bg-white rounded-lg shadow-md p-3 sm:p-4 border-l-4 border-orange-500">
                ...
                <p className="text-xl sm:text-2xl font-bold text-orange-600">{totalRemindersFiltered}</p>
                <p className="text-xs text-gray-500 mt-1">
                  {pendingRemindersFiltered.length} pendientes
                </p>
              </div>
              ...
              <div className="bg-white rounded-lg shadow-md p-3 sm:p-4 border-l-4 border-red-500">
                ...
                <p className="text-lg sm:text-2xl font-bold text-red-600">${formatCurrency(totalPendingFiltered)}</p>
              ...
              <div className="bg-white rounded-lg shadow-md p-3 sm:p-4 border-l-4 border-yellow-500">
                ...
                <p className="text-xl sm:text-2xl font-bold text-yellow-600">{dueSoonRemindersFiltered.length}</p>
                <p className="text-xs text-gray-500 mt-1">
                  ${formatCurrency(dueSoonRemindersFiltered.reduce((sum, r) => sum + r.amount, 0))}
                </p>
              </div>
              ...
              <div className="bg-white rounded-lg shadow-md p-3 sm:p-4 border-l-4 border-purple-500">
                ...
                <p className="text-xl sm:text-2xl font-bold text-purple-600">{overdueRemindersFiltered.length}</p>
                <p className="text-xs text-gray-500 mt-1">
                  ${formatCurrency(overdueRemindersFiltered.reduce((sum, r) => sum + r.amount, 0))}
                </p>
              </div>
            </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 mb-4">
                ...
                <div>
                  ...
                  >
                    ...
                    <option value="mensual">Mensual (automático día 1)</option>
            ...
            <div className="bg-white rounded-lg shadow-md p-4 sm:p-6">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-4">
                ...
                <div className="flex gap-1 sm:gap-2 flex-wrap w-full sm:w-auto">
                  ...
                  >
                    Todos ({filterRemindersByDate().length})
                  </button>
                  <button
